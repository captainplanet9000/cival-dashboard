-- Phase-2: Live trading schema & RPCs
-- Timestamp: 2025-04-24 08:09 (PDT)
-- NOTICE: All new objects use SECURITY INVOKER and search_path=''.
...
-- triggers
create trigger handle_agents_created_at before insert on public.agents for each row execute function public.handle_created_at();
create trigger handle_agents_updated_at before update on public.agents for each row execute function public.handle_updated_at();
...
create trigger handle_live_orders_created_at before insert on public.live_orders for each row execute function public.handle_created_at();
create trigger handle_live_orders_updated_at before update on public.live_orders for each row execute function public.handle_updated_at();
...
create trigger handle_positions_created_at before insert on public.positions for each row execute function public.handle_created_at();
create trigger handle_positions_updated_at before update on public.positions for each row execute function public.handle_updated_at();
...
create trigger handle_trades_perf_created_at before insert on public.trades_performance for each row execute function public.handle_created_at();
create trigger handle_trades_perf_updated_at before update on public.trades_performance for each row execute function public.handle_updated_at();
...
create trigger handle_risk_config_created_at before insert on public.risk_config for each row execute function public.handle_created_at();
create trigger handle_risk_config_updated_at before update on public.risk_config for each row execute function public.handle_updated_at();
...
create trigger handle_strategy_params_created_at before insert on public.strategy_params for each row execute function public.handle_created_at();
create trigger handle_strategy_params_updated_at before update on public.strategy_params for each row execute function public.handle_updated_at();
...
create trigger handle_agent_health_created_at before insert on public.agent_health for each row execute function public.handle_created_at();
create trigger handle_agent_health_updated_at before update on public.agent_health for each row execute function public.handle_updated_at();
...
create trigger handle_agent_events_created_at before insert on public.agent_events for each row execute function public.handle_created_at();
