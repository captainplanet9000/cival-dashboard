...
import { 
  stopAgent, // Changed from getAgentById
  AgentNotFoundError, 
  AgentForbiddenError, 
  AgentServiceError,
  AgentStatusError // Added for stopAgent
...
export async function POST(request: NextRequest, { params }: { params: { agent_id: string } }) {
  ...
  if (authError || !user) {
    console.error(`POST /api/agents/${agent_id}/stop: Authentication error`, authError);
    return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  }

  try {
    // 2. Call the service function to stop the agent
    // stopAgent internally calls getAgentById for fetch/auth and handles UUID validation
    const updatedAgent = await stopAgent(supabase, user.id, agent_id);
    
    // 3. Success Response
    return NextResponse.json(updatedAgent, { status: 200 });

  } catch (error: any) {
    console.error(`POST /api/agents/${agent_id}/stop: Error - ${error.name} - ${error.message}`);
