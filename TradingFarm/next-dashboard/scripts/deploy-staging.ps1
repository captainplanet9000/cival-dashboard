# Trading Farm Dashboard - Staging Deployment Script
# PowerShell script for deploying to staging environment

Write-Host "╔══════════════════════════════════════════════╗" -ForegroundColor Cyan
Write-Host "║      TRADING FARM STAGING DEPLOYMENT         ║" -ForegroundColor Cyan
Write-Host "╚══════════════════════════════════════════════╝" -ForegroundColor Cyan

# Configuration
$ProjectDir = Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path)
$LogFile = Join-Path -Path $ProjectDir -ChildPath "deployment-staging.log"
$BuildDir = Join-Path -Path $ProjectDir -ChildPath ".next"
$EnvFile = Join-Path -Path $ProjectDir -ChildPath ".env.staging"
...
if (-not (Test-Path $EnvFile)) {
    Handle-Error "Staging environment file not found: $EnvFile"
}

# Begin deployment process
Log-Message "Starting staging deployment process..." "Green"
...
Log-Message "Building application for staging..." "Blue"
try {
...
Log-Message "Deploying to staging..." "Yellow"
...
Log-Message "✓ Deployment to staging completed successfully!" "Green"
...
Add-Content -Path $LogFile -Value "Environment: Staging"
...
Log-Message "Deployment complete! Visit https://staging.tradingfarm.com to verify." "Green"
