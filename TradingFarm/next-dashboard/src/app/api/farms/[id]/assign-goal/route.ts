...
export async function POST(
  ...
  try {
    ...
    const { goal_id, agent_id, is_eliza_agent } = requestData;

    if (!farmId || !goal_id || !agent_id) {
      return NextResponse.json(
        { error: "Farm ID, goal ID, and agent ID are required" },
        { status: 400 }
      );
    }
    ...
    // Update agent with goal reference
    ...
    if (updateError) {
      return NextResponse.json(
        { error: "Failed to assign goal to agent" },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json({ 
      data: { 
        message: "Goal assigned to agent successfully" 
      } 
    });
  } catch (error) {
