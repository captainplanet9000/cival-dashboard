/**
 * Stop Trading Strategy API Endpoint
 * 
 * Deactivates a trading strategy and stops real-time trading
 */
...
export async function POST(
  ...
  try {
    ...
    // If strategy is already stopped, return success
    if (!strategy.isActive) {
      return NextResponse.json({ success: true, message: 'Strategy is already stopped' });
    }
    
    // Stop the strategy in the database
    ...
    // Call the strategy service to stop the strategy
    const stopped = await tradingStrategyService.stopStrategy(strategyId);
    ...
    return NextResponse.json({ 
      success: stopped,
      message: stopped ? 'Strategy stopped successfully' : 'Failed to stop strategy'
    });
  } catch (error) {
    console.error('Unexpected error in stop strategy:', error);
