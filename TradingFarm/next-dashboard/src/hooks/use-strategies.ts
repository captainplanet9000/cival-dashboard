...
export function useStrategies() {
  ...
  const fetchStrategies = useCallback(async () => {
    try {
      ...
      console.log('useStrategies: Fetching strategies data...');
      const data = await strategyService.getStrategies();
      
      if (data && Array.isArray(data)) {
        console.log(`useStrategies: Successfully fetched ${data.length} strategies`);
        setStrategies(data);
      } else {
        console.warn('useStrategies: Received invalid data format:', data);
        setStrategies([]);
      }
      ...
      setError(`Unable to load strategies. ${errorMessage}`);
      setLoading(false);
      
      // Even on error, we're considered initialized
      setInitialized(true);
    }
  }, []);
