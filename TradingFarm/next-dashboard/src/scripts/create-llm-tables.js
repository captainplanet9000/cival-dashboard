/**
 * Create LLM Integration Tables Script
 * 
 * This script creates the necessary database tables for LLM integration
 * in the Trading Farm system. It's an alternative to using Supabase migrations
 * which were causing configuration issues.
 */
...
// SQL statements to create the necessary database structure
const createApiServiceTypeEnum = `
  DO $$
  BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'api_service_type') THEN
...
const createApiServiceProvidersTable = `
  CREATE TABLE IF NOT EXISTS public.api_service_providers (
...
const createApiServiceProvidersRLS = `
  DO $$
  BEGIN
    IF NOT EXISTS (
      SELECT 1 
      FROM pg_catalog.pg_policies 
      WHERE tablename = 'api_service_providers' 
...
const createUserApiConfigurationsTable = `
  CREATE TABLE IF NOT EXISTS public.user_api_configurations (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
const createUserApiConfigurationsRLS = `
  DO $$
  BEGIN
    IF NOT EXISTS (
      SELECT 1 
      FROM pg_catalog.pg_policies 
      WHERE tablename = 'user_api_configurations' 
      AND policyname = 'Users can CRUD their own API configurations'
    ) THEN
...
const createAgentApiServicesTable = `
  CREATE TABLE IF NOT EXISTS public.agent_api_services (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
const createAgentApiServicesRLS = `
  DO $$
  BEGIN
    IF NOT EXISTS (
      SELECT 1 
      FROM pg_catalog.pg_policies 
      WHERE tablename = 'agent_api_services' 
      AND policyname = 'Users can manage API services for agents they own'
    ) THEN
...
const createApiServiceUsageLogsTable = `
  CREATE TABLE IF NOT EXISTS public.api_service_usage_logs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    configuration_id UUID REFERENCES public.user_api_configurations(id) ON DELETE CASCADE,
...
const createApiServiceUsageLogsRLS = `
  DO $$
  BEGIN
    IF NOT EXISTS (
      SELECT 1 
      FROM pg_catalog.pg_policies 
      WHERE tablename = 'api_service_usage_logs' 
      AND policyname = 'Users can view their own API usage logs'
    ) THEN
...
const createTriggers = `
  ...
  BEGIN
    IF NOT EXISTS (
      SELECT 1 
      FROM pg_trigger 
      WHERE tgname = 'update_api_service_providers_updated_at'
    ) THEN
    ...
    IF NOT EXISTS (
      SELECT 1 
      FROM pg_trigger 
      WHERE tgname = 'update_user_api_configurations_updated_at'
    ) THEN
    ...
    IF NOT EXISTS (
      SELECT 1 
      FROM pg_trigger 
      WHERE tgname = 'update_agent_api_services_updated_at'
    ) THEN
...
const alterLlmConfigsTable = `
  ...
  BEGIN
    IF EXISTS (
      ...
      WHERE table_name = 'llm_configs' 
      AND column_name = 'api_configuration_id'
    ) THEN
      ALTER TABLE public.llm_configs ADD COLUMN api_configuration_id UUID REFERENCES public.user_api_configurations(id) ON DELETE SET NULL;
    END IF;
  END
  $$;
`;

const insertDefaultProviders = `
  INSERT INTO public.api_service_providers (name, service_type, description, config_schema) 
  VALUES
  -- LLM Providers
...
// Define the main function to run all SQL statements
async function createLlmTables() {
  try {
    console.log('Starting LLM tables creation...');

    // Execute each SQL statement in sequence
    console.log('Creating API service type enum...');
    await supabase.rpc('pgqlu', { query: createApiServiceTypeEnum });

    console.log('Creating API service providers table...');
    await supabase.rpc('pgqlu', { query: createApiServiceProvidersTable });
    
    console.log('Creating API service providers RLS policies...');
    await supabase.rpc('pgqlu', { query: createApiServiceProvidersRLS });
    
    console.log('Creating user API configurations table...');
    await supabase.rpc('pgqlu', { query: createUserApiConfigurationsTable });
    
    console.log('Creating user API configurations RLS policies...');
    await supabase.rpc('pgqlu', { query: createUserApiConfigurationsRLS });
    
    console.log('Creating agent API services table...');
    await supabase.rpc('pgqlu', { query: createAgentApiServicesTable });
    
    console.log('Creating agent API services RLS policies...');
    await supabase.rpc('pgqlu', { query: createAgentApiServicesRLS });
    
    console.log('Creating API service usage logs table...');
    await supabase.rpc('pgqlu', { query: createApiServiceUsageLogsTable });
    
    console.log('Creating API service usage logs RLS policies...');
    await supabase.rpc('pgqlu', { query: createApiServiceUsageLogsRLS });
    
    console.log('Creating updated_at function...');
    await supabase.rpc('pgqlu', { query: createUpdatedAtFunction });
    
    console.log('Creating triggers...');
    await supabase.rpc('pgqlu', { query: createTriggers });
    
    console.log('Altering LLM configs table...');
    await supabase.rpc('pgqlu', { query: alterLlmConfigsTable });
    
    console.log('Inserting default providers...');
    await supabase.rpc('pgqlu', { query: insertDefaultProviders });
    
    console.log('LLM tables created successfully!');
  } catch (error) {
    console.error('Error creating LLM tables:', error);
  }
}
