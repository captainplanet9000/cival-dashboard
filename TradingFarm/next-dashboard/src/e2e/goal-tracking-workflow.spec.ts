...
test.describe('Goal Tracking Dashboard Workflow', () => {
  ...
  test('should display goal details and progress charts', async ({ page }) => {
    ...
    // Wait for the page to load
    await page.waitForSelector('h1:has-text("Goal Tracking")');
  ...
  test('should allow filtering goals by farm', async ({ page }) => {
    // Navigate to the goals tracking page
    await page.goto('/dashboard/goals/tracking');
    ...
    // Navigate to a URL with a farmId parameter
    await page.goto('/dashboard/goals/tracking?farmId=1');
    
    // Verify the page still loads with farm-filtered goals
    await expect(page.getByText('Goal Tracking')).toBeVisible();
    
    // The URL should maintain the farmId parameter
    await expect(page.url()).toContain('farmId=1');
  });
  ...
  test('should refresh data when requested', async ({ page }) => {
    ...
    // Expect to see some loading indicator or data refresh
    // This might be difficult to assert without specific loading indicators
    
    // Verify the page still shows the key components after refresh
    await expect(page.getByText('Goal Tracking')).toBeVisible();
    await expect(page.getByText('Active Goals')).toBeVisible();
  });
