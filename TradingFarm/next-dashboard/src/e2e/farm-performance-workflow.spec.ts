...
test.describe('Farm Performance Dashboard Workflow', () => {
  ...
  test('should switch between tabs on the performance dashboard', async ({ page }) => {
    ...
    // Wait for the page to load
    await page.waitForSelector('h1:has-text("Farm Performance")');
  ...
  test('should refresh data when requested', async ({ page }) => {
    ...
    // Expect to see some loading indicator (could be an animation on the button)
    // Note: This might require adding a data-testid or similar to properly identify the loading state
    
    // Verify that the page reloads its data successfully
    await expect(page.getByText('Performance Trend')).toBeVisible();
  });
});
