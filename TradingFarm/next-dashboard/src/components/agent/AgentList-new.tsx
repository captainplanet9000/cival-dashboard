...
export function AgentList({
  ...
  const startAgent = useCallback(async (agentId: string) => {
    try {
      ...
      // Log the event
      await healthMonitor.reportEvent(agentId, 'agent_started', 'Agent started successfully');
  ...
  }, [supabase, healthMonitor, refreshAgents, toast]);
  ...
  const stopAgent = useCallback(async (agentId: string) => {
    try {
      ...
      // Log the event
      await healthMonitor.reportEvent(agentId, 'agent_stopped', 'Agent stopped successfully');
  ...
  const restartAgent = useCallback(async (agentId: string) => {
    try {
      ...
      // Log the event
      await healthMonitor.reportEvent(agentId, 'agent_restarted', 'Agent restarted successfully');
            ...
            <CardContent>
              <div className="space-y-4">
                {healthLoading ? (
                  ...
                  <div className="space-y-4">
                    {agents
                      .filter(a => agentHealth[a.id]?.status === 'critical' || agentHealth[a.id]?.status === 'warning')
                      .sort((a, b) => {
