...
const EntryNode = memo(({ data, selected }: NodeProps<EntryNodeData>) => {
  ...
  let borderColor = selected ? 'border-green-500' : 'border-green-200 dark:border-green-800';
  ...
  return (
    <div className={`${bgColor} ${borderColor} p-3 rounded-md border shadow-sm min-w-[180px]`}>
      ...
      <div className="flex items-center">
        <DirectionIcon className="w-4 h-4 mr-2" />
      ...
      {data.parameters?.orderType && (
        <div className="text-xs mt-2 px-2 py-1 bg-background rounded">
          Order: {data.parameters.orderType.replace('_', ' ')}
        </div>
      )}
