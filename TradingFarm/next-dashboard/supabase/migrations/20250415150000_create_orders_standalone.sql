...
-- Create a minimal orders table that doesn't depend on other tables
-- This creates a functioning orders table without foreign key constraints
-- You can add those constraints later when the referenced tables exist
...
BEGIN
    ...
    -- This ensures it will work regardless of what other tables exist
    ...
    ELSE
        RAISE NOTICE 'handle_created_at function not found, skipping trigger creation';
    END IF;
    ...
    -- Create a simple RLS policy without complex references
    ...
    -- Output a message about next steps
    RAISE NOTICE 'NOTE: Foreign key constraints were not added to ensure successful creation.';
    RAISE NOTICE 'After creating the referenced tables, you can add constraints with ALTER TABLE statements.';
END
$$;
